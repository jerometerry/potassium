namespace Sodium
{
    ...
    public interface IObservable<T> : IDisposable
    {
        ...
        /// <param name="callback">An Action to be invoked when the current Event publishes.</param>
        /// <returns>An ISubscription, that should be Disposed when no longer needed. </returns>
        ISubscription<T> Subscribe(IPublisher<T> callback);
        ...
        ISubscription<T> Subscribe(IPublisher<T> callback, Rank subscriptionRank);
        ...
        ISubscription<T> Subscribe(IPublisher<T> callback, Rank superior, Transaction transaction);
